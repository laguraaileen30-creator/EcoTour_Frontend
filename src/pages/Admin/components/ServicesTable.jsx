...
import { Plus, Edit2, Trash2, Tag, RefreshCw, RotateCw, Image as ImageIcon } from 'lucide-react';
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-[#071911] p-5 rounded-2xl border border-emerald-500/20 shadow-xl gap-4">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-0.5">
            Interactive 3D Cards — Flip to view full details, capacity, pricing, and management tools.
          </p>
        </div>
                ...
                <div className="absolute inset-0 h-full w-full rounded-2xl overflow-hidden [backface-visibility:hidden] border border-emerald-500/20 bg-slate-900 group">
                  ...
                  <div className="absolute top-3 left-3 right-3 flex justify-between items-center z-10">
                    ...
                    <span className={`px-2.5 py-1 rounded-full text-[10px] font-extrabold uppercase tracking-wider backdrop-blur-md shadow-md ${
                      srv.status === 'Active' ? 'bg-emerald-500/90 text-white border border-emerald-300/40' : 'bg-slate-700/90 text-slate-200 border border-slate-500/40'
                    }`}>
                      {srv.status}
                    </span>
                  </div>
                ...
                <div className="absolute inset-0 h-full w-full rounded-2xl p-5 [backface-visibility:hidden] [transform:rotateY(180deg)] bg-[#05180f] border border-emerald-500/30 text-white flex flex-col justify-between shadow-2xl">
                  ...
                  <div>
                    ...
                    <div className="mt-3 space-y-1">
                      ...
                      <p className="text-xs text-slate-200 leading-relaxed line-clamp-4 bg-black/30 p-2.5 rounded-xl border border-white/5">
                  ...
                  <div className="space-y-2 my-2 py-2 border-y border-emerald-900/60">
                    <div className="grid grid-cols-2 gap-2 text-xs">
                      ...
                      <div className="bg-black/30 p-2 rounded-lg border border-white/5">
                        ...
                        <span className="font-bold text-slate-200 text-xs">{srv.total_capacity || 10} Total Units</span>
                        <span className="text-[10px] font-extrabold text-emerald-300 block mt-0.5">
                          {srv.available_qty !== undefined ? srv.available_qty : (srv.available_quantity !== undefined ? srv.available_quantity : srv.total_capacity || 10)} Available
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="space-y-1 bg-black/40 p-2 rounded-xl border border-emerald-900/40">
                      <div className="flex justify-between text-[10px]">
                        ...
                        <span className="font-extrabold text-emerald-300">
                          {srv.available_qty !== undefined ? srv.available_qty : (srv.available_quantity !== undefined ? srv.available_quantity : srv.total_capacity || 10)} / {srv.total_capacity || 10} Ready
                        </span>
                      </div>
                      <div className="h-1.5 w-full bg-black/60 rounded-full overflow-hidden border border-white/5">
                        <div
                          ...
                          style={{
                            width: `${Math.min(100, ((((srv.available_qty !== undefined ? srv.available_qty : (srv.available_quantity !== undefined ? srv.available_quantity : srv.total_capacity || 10))) / (srv.total_capacity || 10)) * 100))}%`
                          }}
                        />
                      </div>
                    </div>
